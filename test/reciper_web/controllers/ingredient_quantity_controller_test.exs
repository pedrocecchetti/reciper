defmodule ReciperWeb.IngredientQuantityControllerTest do
  ...
  @create_ingredient_attrs %{name: "Brocolli", category: :vegetable}
  @create_attrs %{measurement: :tbsp, quantity: 42}
  ...
  describe "create ingredient_quantity" do
    test "redirects to show when data is valid", %{conn: conn} do
      conn = post(conn, ~p"/ingredients", ingredient: @create_ingredient_attrs)
      %{id: ingredient_id} = redirected_params(conn)
      create_attrs = Map.put(@create_attrs, :ingredient_id, ingredient_id)
  ...
  describe "edit ingredient_quantity" do
    ...
    test "renders form for editing chosen ingredient_quantity", %{
      conn: conn,
  ...
  describe "update ingredient_quantity" do
    ...
    test "redirects when data is valid", %{conn: conn, ingredient_quantity: ingredient_quantity} do
      conn =
        put(conn, ~p"/ingredient_quantities/#{ingredient_quantity}",
          ingredient_quantity: @update_attrs
        )
    ...
    test "renders errors when data is invalid", %{
      ...
      conn =
        put(conn, ~p"/ingredient_quantities/#{ingredient_quantity}",
          ingredient_quantity: @invalid_attrs
        )
  ...
  describe "delete ingredient_quantity" do
    ...
    test "deletes chosen ingredient_quantity", %{
      conn: conn,
      ingredient_quantity: ingredient_quantity
    } do
